...
def DCT(M,compressionDCT): #applique la DCT à des blocs de 8*8 Attention ne marche que avec des matrices de taille mutiples de 8 (n*8) !
    x,y =M.shape
    ...
    for i in range (q): #def la sous 
        ...
        for j in range (q): 
            ...
            mDCT=scifft.dctn(m,norm = 'ortho') #calcul de sa DCT (domaine fréquentiel)
            compressionDCT(mDCT) #compression selon la methode utilisé dans la fct 
            add_matrix(F, n, c, mDCT, p,p) #ajout dans la matrice finale 
            
    return(F)

def DCTi(M): #applique la DCT inverse à des blocs de 8*8 Attention ne marche que avec des matrices de taille mutiples de 8 (n*8) !
    x,y =M.shape
    assert x==y #vérifie que l'image est bien carrée 
    F=np.zeros((x,y))
    q=x//8
    ...
    for i in range (q): #def la sous 
        ...
        for j in range (q): 
            c=j*8 #def le debut de la colonne la sous matrice à prendre
            m=M[n:n+8,c:c+8]#sous matrice de taille 8*8
            mDCTi=scifft.idctn(m,norm = 'ortho') #calacul de sa DCT inverseremise dans le domaine spatial
            add_matrix_rounded(F, n, c, mDCTi, p,p) #ajout dans la matrice finale 
    return(F)

def FT(M,compressionFT):
    x,y =M.shape
    assert x==y #vérifie que l'image est bien carrée 
    ...
    for i in range (q): #def la sous 
        ...
        for j in range (q): 
            ...
            mFT=np.fft.fft2(m) #calcul de sa FT (domaine fréquentiel)
            #np.fft.fftshift(fftimage)
            compressionFT(mFT) #compression selon la methode utilisé dans la fct 
