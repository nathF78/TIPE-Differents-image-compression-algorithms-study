...
def DCT(M,compressionDCT): #applique la DCT à des blocs de 8*8 
    x,y =M.shape
    ...
    for i in range (q): #def la sous 
        ...
        for j in range (q): 
            ...
            compressionDCT(mDCT) #compression selon la fct 
...
def FTi(M):
    ...
    for i in range (q): #def la sous 
        ...
        for j in range (q): 
            c=j*8 #def le debut de la colonne la sous matrice à prendre
            m=M[n:n+8,c:c+8]#sous matrice de taille 8*8
            mFTi=np.real(np.fft.ifft2(m)) #calacul de sa DCT inverse remise dans le domaine spatial
